<div class="space-y-6">
    ...
    ══════════════════════════════════════════════════════ --}}
    @if($showQuiz && $activePracticeId)
    <div class="fixed inset-0 z-50 bg-[#F8FAFC] dark:bg-slate-950 overflow-y-auto">
        <div class="max-w-3xl mx-auto px-4 py-8 space-y-6">

            {{-- Quiz Header --}}
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-xl font-extrabold text-slate-900 dark:text-white">Practice Quiz</h2>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ count($activeQuestions) }} questions — select one answer per question</p>
                </div>
                <button wire:click="exitQuiz" class="inline-flex items-center gap-2 rounded-2xl border border-slate-200 dark:border-slate-800 px-4 py-2 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900 transition">
                    ✕ Exit Quiz
                </button>
            </div>
            ...
            @if($submitted)
                ...
                <div class="rounded-3xl p-6 text-center
                    {{ $pct >= 70 ? 'bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800' : 'bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800' }}">
                    <p class="text-4xl font-extrabold {{ $pct >= 70 ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300' }}">{{ $pct }}%</p>
                    <p class="text-sm font-bold mt-1 {{ $pct >= 70 ? 'text-emerald-800 dark:text-emerald-200' : 'text-amber-800 dark:text-amber-200' }}">
                        {{ $score }} / {{ $total }} correct
                        @if($pct >= 70) — Great job! 🎉 @else — Keep practising! 💪 @endif
                    </p>
                    <button wire:click="exitQuiz" class="mt-4 rounded-2xl bg-indigo-600 hover:bg-indigo-700 px-5 py-2 text-sm font-bold text-white shadow transition">Done</button>
                </div>
            @endif

            {{-- Questions --}}
            <div class="space-y-6">
                @foreach($activeQuestions as $qi => $question)
                    @php
                        $resultItem = collect($results)->firstWhere('id', $question->id);
                    @endphp
                    <div class="rounded-3xl border
                        {{ $submitted ? ($resultItem['is_correct'] ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50/30 dark:bg-emerald-950/10' : 'border-rose-200 dark:border-rose-800 bg-rose-50/30 dark:bg-rose-950/10') : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900' }}
                        p-6 shadow-sm">

                        <div class="flex items-start gap-3 mb-4">
                            <span class="size-7 shrink-0 rounded-xl bg-indigo-600 text-white text-xs font-bold flex items-center justify-center">{{ $qi + 1 }}</span>
                            <p class="text-sm font-bold text-slate-900 dark:text-white leading-snug">{{ $question->question }}</p>
                        </div>

                        @if($question->topic)
                            <span class="inline-block mb-3 rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-[10px] font-bold text-slate-500 uppercase tracking-wide">{{ $question->topic }}</span>
                        @endif
                        ...
                        <div class="space-y-2.5">
                            @foreach($question->options->sortBy('option_label') as $option)
                                @php
                                    ...
                                    $baseClass  = 'flex items-center gap-3 w-full rounded-2xl border px-4 py-3 text-sm font-semibold text-left transition';
                                    if ($submitted) {
                                        if ($isCorrect) $btnClass = "$baseClass border-emerald-500 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-200";
                                        elseif ($isSelected && !$isCorrect) $btnClass = "$baseClass border-rose-400 bg-rose-50 dark:bg-rose-950/40 text-rose-800 dark:text-rose-200";
                                        else $btnClass = "$baseClass border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400";
                                    } else {
                                        $btnClass = $isSelected
                                            ? "$baseClass border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-800 dark:text-indigo-200"
                                            : "$baseClass border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-indigo-300 hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20";
                                    }
                                @endphp
                                <button type="button"
                                    ...
                                    class="{{ $btnClass }}"
                                    {{ $submitted ? 'disabled' : '' }}>
                                    <span class="size-7 shrink-0 rounded-xl border text-xs font-black flex items-center justify-center
                                        {{ $submitted && $isCorrect ? 'border-emerald-500 bg-emerald-500 text-white' : ($submitted && $isSelected && !$isCorrect ? 'border-rose-400 bg-rose-400 text-white' : ($isSelected ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-slate-300 dark:border-slate-600 text-slate-500')) }}">
                                        {{ $option->option_label }}
                                    </span>
                                    {{ $option->option_text }}
                                </button>
                        ...
                        @if($submitted && $resultItem && $resultItem['explanation'])
                            <div class="mt-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 p-4 text-xs text-slate-700 dark:text-slate-300 leading-relaxed">
                                <span class="font-bold text-slate-900 dark:text-white">💡 Explanation: </span>{{ $resultItem['explanation'] }}
                            </div>
                        @endif
                    </div>
                @endforeach
            ...
            @if(!$submitted)
                <div class="flex justify-center pb-8">
                    <button wire:click="submitQuiz"
                        class="inline-flex items-center gap-2 rounded-2xl bg-indigo-600 hover:bg-indigo-700 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-indigo-600/25 transition">
