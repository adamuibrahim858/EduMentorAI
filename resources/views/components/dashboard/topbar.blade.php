...
<header class="sticky top-0 z-20 flex h-20 w-full items-center justify-between border-b border-slate-200/80 bg-white/80 px-4 sm:px-8 backdrop-blur-xl transition-all duration-300 dark:border-slate-800/80 dark:bg-slate-900/80">
    ...
    <div class="flex items-center gap-3">
        ...
        @php
            ...
            $topUnreadCount  = auth()->check() ? $notifService->getUnreadCount() : 0;
            $topRecentNotifs = auth()->check() ? $notifService->getRecent(limit: 5) : collect();
        @endphp
        <div x-data="{ notifMenuOpen: false }" class="relative">
            <button 
            ...
            >
                ...
                @if($topUnreadCount > 0)
                    <span class="absolute -top-1 -right-1 flex size-5 items-center justify-center rounded-full bg-emerald-500 text-[10px] font-extrabold text-white ring-2 ring-white dark:ring-slate-900">
                        {{ $topUnreadCount > 9 ? '9+' : $topUnreadCount }}
                    </span>
                @endif
            </button>
            ...
            <div 
                ...
                class="absolute right-0 mt-3 w-80 sm:w-96 rounded-3xl border border-slate-200/80 bg-white shadow-2xl backdrop-blur-xl dark:border-slate-800 dark:bg-slate-900 z-50 overflow-hidden"
                style="display: none;"
            >
                <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50">
                    ...
                    <a href="{{ route('notifications.index') }}" class="text-xs font-bold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                        View All
                    </a>
                </div>

                <div class="max-h-80 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60">
                    @forelse($topRecentNotifs as $n)
                        <a 
                            href="{{ $n->action_url ?? route('notifications.index') }}" 
                            class="flex items-start gap-3 p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition {{ !$n->is_read ? 'bg-indigo-50/30 dark:bg-indigo-950/20' : '' }}"
                        >
                            @include('livewire.notification._icon', ['type' => $n->type])
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2">{{ $n->message }}</p>
                ...
                <div class="p-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 text-center">
                    <a 
                        href="{{ route('notifications.index') }}" 
                        class="block w-full rounded-xl bg-indigo-50 dark:bg-indigo-950/60 py-2 text-xs font-bold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 transition"
                    >
